...
import com.sdk.oms.shopee.core.api.exceptions.RestClientBaseException;
...
import com.sdk.oms.shopee.core.transport.*;
...
import static com.sdk.oms.shopee.core.transport.HttpRequest.RequestBuilder;
...
public class BaseRestClientService {
    ...
    private <R> Invocation<R> builder(Class<R> returnType, String path, HttpMethod method) {
        ...
        if (ses == null) {
            throw new RestClientBaseException("Unable to retrieve current session. Please verify thread has a current session available.");
        }
    ...
    protected static class Invocation<R> {
        ...
        public R execute() {
            header(ClientConstants.HEADER_USER_AGENT, ClientConstants.USER_AGENT);
        ...
        public R executeWithErrorResponse() {
            ...
            if (response instanceof ErrorBaseResponse) {
                ...
                if (null != errorBase.getError()) throw HttpExceptionHandler.mapException(errorBase);
            }
            return response;
        }
